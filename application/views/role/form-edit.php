...
if($roles->num_rows() > 0)
{
  $result = $roles->result_array()[0];
}

$permission_id = array();

if($role_permission->num_rows() > 0)
{
  foreach($role_permission->result_array() as $key => $val)
  {
        ...
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              ...
              <form action="<?= base_url('role/update/'.$id); ?>" id="form" class="form-horizontal" method="POST">
                <div class="card-body">
                  <div class="form-body" style="margin-left: 2em">
                    <div class="form-group">
                      <label class="control-label col-md-3">Name</label>
                      <div class="col-md-12">
                        <input name="name" placeholder="Name" class="form-control" type="text" value="<?= isset($result['name']) && !empty($result['name']) ? $result['name']:''?>">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-12">
                        <input name="description" placeholder="Description" class="form-control" type="text" value="<?= isset($result['description']) && !empty($result['description']) ? $result['description']:''?>">
                        <span class="help-block"></span>
                      </div>
                    </div>
                    <div class="form-group">
                          ...
                          <?php if($permissions->num_rows() > 0){?>
                            <?php foreach($permissions->result_array() as $key => $val){?>
                              <div class="icheck-primary">
                                <input type="checkbox" name="permissions[]" id="checkboxPrimary_<?=$key?>" value="<?=$val['id']?>" <?=in_array($val['id'],$permission_id)?'checked':'' ?> >
