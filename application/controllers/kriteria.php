...
class Kriteria extends MY_Controller {
  ...
  public function getData()
  {
    $data = array(
        ...
        {
          $sql .= "WHERE kriteria LIKE '%".$data['search']."%' OR rangking LIKE '%".$data['search']."%'";
        }
        $sql .= " LIMIT ".$data['offset'].",".$data['limit']; 

        $query = $this->db->query($sql);

        $total = $this->db->get('kriteria')->num_rows();
        ...
        if($query->num_rows() > 0)
        {
          ...
          {
            $isi_tabel[$key]['kriteria'] = $val->kriteria;
  ...
  {
    $sql = "SELECT * FROM kriteria WHERE id ='".$id."' LIMIT 1";
  ...
  public function update()
  {
    ...
    $this->db->update('kriteria', $data, array('id' => $this->input->post('id')));
